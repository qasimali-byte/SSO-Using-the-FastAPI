...
class Query:
    ...
    async def user_actions(self, first: int = 10, cursor: Optional[Cursor] = None, direction: DirectionEnum = "forward",
        search: Optional[str] = None, role_name: Optional[RoleEnum] = None,action_level: Optional[ActionLevelEnum]=None,
        start_date_time:Optional[str]=None,end_date_time:Optional[str]=None) -> Connection[UserAction]:
        
        async with get_session() as s:
            return await get_user_actions(db=s,first=first,cursor=cursor,direction=direction,search=search,
